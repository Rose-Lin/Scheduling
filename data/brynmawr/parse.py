...
class parser:
    def bmc_parse_prof_rooms_times_class(self, file):
        """" A function used on bmc data, returns professors list, rooms list and time slots list"""
        with open(file) as f:
            ...
            profs = {}
            # bmc classes excluding labs and bmc classes
            hc_classes = []
    ...
    def bmc_parse_pref(self, file, hc_classes):
        """" A function used on bmc data, returns professors students pref dict"""
        pref_dict = {}
        with open(file) as f:
            raw_content = f.read().strip()
            lines = raw_content.split('\n')
